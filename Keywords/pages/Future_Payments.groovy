...
public class Future_Payments {
	...
	def verifyInvoiceRecord(TestObject nextObj,TestObject numberOfPages,String invoice,String amount){
		try{
			...
			println map_TableCount['Invoice Number']
			boolean foundValue=false
			boolean lastpage=false
			WebDriver driver=DriverFactory.getWebDriver()
			WebElement table
			while(!lastpage && !foundValue){
				table= driver.findElement(By.xpath("//table[@id='futurePaymentsTable']/tbody"))
				...
				for(int i=0;i<rows.size();i++){
					...
					println amount
					println invoice
					if(cellInvoiceValue.equalsIgnoreCase(invoice) && cellAmountValue.equalsIgnoreCase(amount)){
						safeActions.highLightElement(cols.get(map_TableCount['Invoice Number']), GlobalVariable.delayBetweenTestSteps)
			...
			WebUI.takeScreenshot(safeActions.reportsFolderPath+"/verifyInvoiceRecord_FuturePayments.png")
			KeywordUtil.markFailed("unable to verify InvoiceRecord in Future Payments Screen")
		}
	}
	//}
	...
	def getInvoiceColumnCount(){
		try{
			...
			WebDriver driver=DriverFactory.getWebDriver()
			WebElement table= driver.findElement(By.xpath("//table[@id='futurePaymentsTable']/thead"))
			List<WebElement> rows=table.findElements(By.tagName('tr'))
			for(int i=0;i<rows.size();i++){
				List<WebElement> cols=rows.get(i).findElements(By.tagName('th'))
				for(int j=0;j<cols.size();j++){
					if(cols.get(j).getText().equalsIgnoreCase("Invoice Number")){
						map_TableCount['Invoice Number']=j
					}
			...
			WebUI.takeScreenshot(safeActions.reportsFolderPath+"/getInvoiceColumnCount_FurturePayments.png")
			KeywordUtil.markFailed("unable to get InvoiceRecord column number in Future Payments Screen")
		}
